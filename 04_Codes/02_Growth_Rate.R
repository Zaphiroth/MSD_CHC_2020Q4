...
## National
growth.national <- raw.100 %>% 
  filter(Quarter %in% c('2020Q3', '2020Q4'), 
         !is.na(packcode)) %>% 
  ...
  mutate(growth_value = case_when(packcode %in% c('0217904', '0254702', '1513112', '1693208', '1913004', '3638302', '4369902', '5518902', '6500302') ~ growth_rmb, 
                                  TRUE ~ growth_value), 
         growth_volume = case_when(packcode %in% c('0217904', '0254702', '1513112', '1693208', '1913004', '3638302', '4369902', '5518902', '6500302') ~ growth_unit, 
                                   TRUE ~ growth_volume), 
         growth_dosage = case_when(packcode %in% c('0217904', '0254702', '1513112', '1693208', '1913004', '3638302', '4369902', '5518902', '6500302') ~ growth_su, 
                                   TRUE ~ growth_dosage)) %>% 
  select(city_en, packid = packcode, growth_value, growth_volume, growth_dosage)
...
growth.5 <- raw.100 %>% 
  ...
  mutate(city = gsub('市', '', City), 
         ...
         growth_volume = if_else(is.na(growth_volume), growth_unit, growth_volume), 
         growth_dosage = if_else(is.na(growth_dosage), growth_su, growth_dosage)) %>% 
  mutate(growth_value = case_when(city == '北京' & packcode %in% c('1513112', '4778502', '6689708') ~ growth_rmb, 
                                  city == '苏州' & packcode %in% c('1913004', '4369902', '5518902', '5518904', '6500302', '6820202') ~ growth_rmb, 
                                  city == '南京' & packcode %in% c('1581706', '6820202') ~ growth_rmb, 
                                  city == '宁波' & packcode %in% c('1135402') ~ growth_rmb, 
                                  TRUE ~ growth_value), 
         growth_volume = case_when(city == '北京' & packcode %in% c('1513112', '4778502', '6689708') ~ growth_unit, 
                                   city == '苏州' & packcode %in% c('1913004', '4369902', '5518902', '5518904', '6500302', '6820202') ~ growth_unit, 
                                   city == '南京' & packcode %in% c('1581706', '6820202') ~ growth_unit, 
                                   city == '宁波' & packcode %in% c('1135402') ~ growth_unit, 
                                   TRUE ~ growth_volume), 
         growth_dosage = case_when(city == '北京' & packcode %in% c('1513112', '4778502', '6689708') ~ growth_su, 
                                   city == '苏州' & packcode %in% c('1913004', '4369902', '5518902', '5518904', '6500302', '6820202') ~ growth_su, 
                                   city == '南京' & packcode %in% c('1581706', '6820202') ~ growth_su, 
                                   city == '宁波' & packcode %in% c('1135402') ~ growth_su, 
                                   TRUE ~ growth_dosage)) %>% 
  select(city, packid = packcode, growth_value, growth_volume, growth_dosage)

## Shanghai, Fuzhou
growth.2 <- raw.cpa %>% 
  filter(CITY %in% c('上海市', '福州市')) %>% 
  ...
  full_join(merge(data.frame(City = c('上海市', '福州市')), 
                  growth.chpa), 
            by = c('CITY' = 'City', 'packid' = 'Pack_ID')) %>% 
  mutate(city = gsub('市', '', CITY), 
         growth_value = if_else(is.na(growth_value), growth_rmb, growth_value), 
         growth_dosage = if_else(is.na(growth_dosage), growth_su, growth_dosage), 
         growth_volume = growth_dosage) %>% 
...
growth.gz <- raw.gz %>% 
  ...
  mutate(growth_value = case_when(packcode %in% c('1581702', '1625704', '1625902', '1865706', '3859102') ~ growth_rmb, 
                                  TRUE ~ growth_value), 
         growth_volume = case_when(packcode %in% c('1581702', '1625704', '1625902', '1865706', '3859102') ~ growth_unit, 
                                   TRUE ~ growth_volume), 
         growth_dosage = case_when(packcode %in% c('1581702', '1625704', '1625902', '1865706', '3859102') ~ growth_unit, 
                                   TRUE ~ growth_dosage)) %>% 
  select(city, packid = packcode, growth_value, growth_volume, growth_dosage)
...
growth.msd <- bind_rows(growth.5, growth.2, growth.gz) %>% 
  left_join(city.en, by = 'city') %>% 
  select(city_en, packid, growth_value, growth_volume, growth_dosage) %>% 
  bind_rows(growth.national)

write.xlsx(growth.msd, '03_Outputs/MSD_CHC_Growth_2020Q4.xlsx')
